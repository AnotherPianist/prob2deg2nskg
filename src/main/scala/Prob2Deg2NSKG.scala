...
object Prob2Deg2NSKG {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val path = s"$rootPath/prob2deg2nskg/scale$scale/${System.currentTimeMillis().toString}"
    val machines = if (args.length > 2) args(2).toInt else 8
    val noise = if (args.length > 3) args(3).toInt else 0.1d
    ...
    val conf = new SparkConf().setAppName("Prob2Deg2NSKG")
